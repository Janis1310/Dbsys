...
(
    ferienwohnungsName VARCHAR PRIMARY KEY CHECK (LENGTH(ferienwohnungsName) > 3),
...
(
    ...
    CONSTRAINT besitzt_fk FOREIGN KEY (ausstatungsbezeichnung) REFERENCES Ausstattung(bezeichnung) ON DELETE CASCADE,
    CONSTRAINT besitzt_fk FOREIGN KEY (ferienwohnungsname) REFERENCES Ferienwohnung(ferienwohnungsname) ON DELETE CASCADE
);
...
(
    ...
    anreise DATE NOT NULL CHECK (anreise < abreise),
    sternanzahl INT CHECK (sternenzahl <= 5) CHECK (sternenzahl > 0), -- kann attribut
    betrag NUMBER NOT NULL CHECK (betrag > 0),
    email VARCHAR2(255), -- Muss ich das auch prüfen? Ist ja schon in Kunde geprüft
    ferienwohnungsname VARCHAR2(255) NOT NULL,
    bewertungsdatum DATE CHECK (bewertungsdatum > anreisedatum), -- kann attribut
